...
const PromoSchema = z.object({
  ...
  dynamic_prefix: z.string().regex(/^[A-Za-z0-9]{2,12}$/),
...
function parseForm(fd: FormData) {
  return PromoSchema.safeParse({
    ...
    dynamic_coupon: fd.get('dynamic_coupon') === 'on',
    dynamic_percent: fd.get('dynamic_percent') ?? 10,
    dynamic_valid_minutes: fd.get('dynamic_valid_minutes') ?? 60,
    dynamic_prefix: fd.get('dynamic_prefix') ?? 'RABAT',
...
function toData(d: z.infer<typeof PromoSchema>) {
  return {
    ...
    priority: d.priority,
    trigger: d.trigger,
    dynamic_coupon: d.dynamic_coupon,
    dynamic_percent: d.dynamic_percent,
    dynamic_valid_minutes: d.dynamic_valid_minutes,
    dynamic_prefix: d.dynamic_prefix.toUpperCase(),
